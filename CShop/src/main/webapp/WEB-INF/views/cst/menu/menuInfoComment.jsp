...
<form action="commentProc.do">
<label> 아이템 아이디<input id="itemId" name="itemId"
	value="${itemList[ind].itemId}">
</label>
<br>
<label> 아이템 이름 <input name="itemName"
	value="${itemList[ind].itemName}">
</label>
<br>
<label> 아이템 가격 <input name="itemPrice"
	value="${itemList[ind].price}">
</label>
<br>
<label> 아이템 인포 <input name="itemInfo"
...
<select name="grade">
	...
	<option value="1">★☆☆☆☆</option>
		...
		<td><input type="text" name="evaluationContents" maxlength="300" style="width: 300px"  placeholder="의견을 140자 이내로 적어주세요" >
		</td>
		<td>회원아이디</td>
		<c:choose>
	...
	<c:forEach var="board" items="${evaluationList}">
		...
		<tr height="35px">
			...
			<td></td>
			<td align="center">			<fmt:formatDate value="${board.evaluationDate }"
	...
	<tr>
		<td colspan="5" align="center"><c:if test="${page <= 1 }">[이전]&nbsp; </c:if>
			<c:if test="${page > 1 }">
				...
				<c:if test="${a!=page }">
					<a href="menuInfo.do?page=${a }">[${a }]</a>&nbsp;</c:if>
